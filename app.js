    ...
    joi.validate(req.body,SchemaValidation,(err, result) => {
        ...
        pool.getConnection(function(err, connection) {
            if(err) {
                ...
                res.render('index', {output: `<div class="alert alert-danger" role="alert">Can't Connect to MySQL Database</div>`});
            } else {
                connection.query('INSERT INTO `form` (`name`,`email`,`phone`,`university`,`faculty`,`academic_year`,`first_choice`,`second_choice`) VALUES ("'+req.body.name+'","'+req.body.email+'","'+req.body.phone+'","'+req.body.university+'","'+req.body.faculty+'","'+req.body.academicyear+'","'+req.body.first_choice+'","'+req.body.second_choice+'")', function(err, users) {
                    if(err){
                    ...
                    {
                        connection.query(`SELECT id FROM form WHERE phone = '${req.body.phone}' LIMIT 1;`, function(err, rows) {
                            if(err) {
                                console.log(`MYSQL DB ERROR: Can't Find User with Phone: ${req.body.phone} \n Error: ${err}`)
                                connection.release();
                            } else {
                                let ID = rows[0].id,
